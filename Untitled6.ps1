...
$Users = Get-ADUser -Filter "Enabled -eq `$true -and PasswordNeverExpires -eq `$false -and PasswordExpired -eq `$false" -SearchBase $OU -Properties "Name", "EmailAddress", "msDS-UserPasswordExpiryTimeComputed" |
    ...
    Where-Object { $_.PasswordExpiry -ge (Get-Date).Date -and $_.PasswordExpiry -le (Get-Date).Date.AddDays(150) }
...
$Mail.Body = "IT would like to inform you that your account password is scheduled to expire on $ExpiryDate.
...
$Mail.To = "" # No direct recipients
$Mail.BCC = ($Users.EmailAddress -join ";")
